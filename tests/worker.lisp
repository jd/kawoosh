...
(def-test join-part-channel ()
  (with-fixture database ()
    ...
    (with-fixture request ("/user/jd/connection/Naquadah/channel"
                           :expected-status-code 204)
      (is (equal nil (read-char stream nil))))
    (with-fixture worker ("jd" "Naquadah")
      (with-fixture request ("/user/jd/connection/Naquadah/channel/%23test"
                             :method :PUT
                             :content "{}"
      ...
      (with-fixture request ("/user/jd/connection/NoConnection/channel/%23test"
        ...
        (is (equal '((:status . "Not Found") (:message . "No such connection"))
                   (decode-json stream))))
      (with-fixture request ("/user/jd/connection/Naquadah/channel/foobar"
                             :method :DELETE
                             :expected-status-code 404)
        (is (equal '((:status . "Not Found")
                     (:message . "No such connection or channel not joined"))
                   (decode-json stream))))
      (with-fixture request ("/user/jd/connection/Naquadah/channel/%23test/events")
        (let* ((s (decode-json stream))
               (event (nth 0 s)))
